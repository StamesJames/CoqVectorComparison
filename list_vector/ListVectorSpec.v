...
- revert n v1 v2; refine (@rect2 _ _ _ _ _).
  ...
  + intros n ? ? H ? ? H0. f_equal.
    ...
    * apply H. 
      intros p1 p2 H1.
      specialize (H0 (Fin.FS p1) (Fin.FS p2) (f_equal (@Fin.FS n) H1)).
      destruct v1, v2.
      cbn in H0.
      enough ({| elts := elts; elts_spec := f_equal Init.Nat.pred (f_equal S elts_spec) |} = {| elts := elts; elts_spec := elts_spec |} /\ {| elts := elts0; elts_spec := f_equal Init.Nat.pred (f_equal S elts_spec0) |} = {| elts := elts0; elts_spec := elts_spec0 |}).
      -- destruct H2.
        rewrite <- H2.
        rewrite <- H3.
        apply H0.
      -- split; apply vec_spec_eq; reflexivity.
- intros; now f_equal.
Qed.
